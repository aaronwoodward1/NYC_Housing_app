...
mortgage_df <- mutate(sales_df, monthly_mort_payment = (value*.80)*(mort_rate/12)/ (1-((1 + mort_rate/12)^(-360)))) %>%
    ...
    tabPanel("Sales Data",
            sidebarLayout(
               sidebarPanel(width = 3, h4("For the Interactive Map"),
                  ...
                  h4("For Mortgage Calculator and Price Forecasts"),
                  selectInput("mort_area", "Select Area:",
                              choices = unique(sales_df$areaName),
                              selected = "NYC"),
                ...
                mainPanel(width = 9,
                         #leafletOutput(outputId = "rent_map", width = 450, height=350),
                         #fluidRow(column(width = 6, h6("Historical Median Asking Rent", align='left'), plotlyOutput("rent_history_plot")),
                         #         column(width = 6, h6("Affordability of New York City Neighborhoods", align='left'), plotlyOutput("rent_afford_plot"))
                         ...
                         #fluidRow(column(width = 6, h4("Estimated Monthly Mortgage Payments and Home Price Forecast", align='left'),h5("Estimated monthly mortgage payment calculation uses the most recent median listing price for the selected area, as well as the average mortgage rate of 5.38% as of October 2023.", align='left'), plotlyOutput("mortgage"))) #h6("Affordability is based on the median household income in New York City of $74,694 in 2022", align='left'), 
               )
            )
        ),
    ...
    sidebarLayout(
      sidebarPanel(width = 3, h4("For the Interactive Map"),
        selectInput("rent_type","Type of Rental:", 
              ...
              #                choices = list("All properties" = "All properties",
              #                            "Studio" = "Studio",
              #                            "1 Bedroom" = "1 Bedroom", 
        ...
        selectInput("rent_metric", "Select Metric:",
              ...
              #                choices = list("Median Asking Rent" = "Median Asking Rent",
              #                           "Rental Inventory" = "Rental Inventory")
        # Input time/date slider
      # sliderInput("slider", "time", "Select month:", 
      #              min = min(rent_df$time),
      #              max = max(rent_df$time),
      #              value = "2022-12-01")
      #),
      selectInput("date", "Select Date:",
...
server <- function(input, output, session){
  ...
  rent_map_data <- reactive({
    filter(rent_sf, type == input$rent_type & metric == input$rent_metric & time == input$date)
  })

  
  #rent_plot_data <- reactive({
  #  filter(rent_df, areaName == input$areaName & type == input$rent_type_plot & metric == input$rent_metric_plot)
  #})
...
#  observe(
#   { click = input$rent_map_shape_click
#   if (is.null(click))
#      return()
#    else
#      leafletProxy("rent_map") %>%
#      setView(lng = click$lng, lat = click$lat, zoom = 13)
#    }
#  )
...
#  metricPlotEventData <- reactive({
#    event_data(
#      event = "rent_map_shape_click",
#      source = "rent_map")
#  })
 ...
 output$rent_afford_plot <- renderPlotly({
   p <- nyc_rent_afford_df %>%
     plot_ly(#nyc_rent_afford_df, 
      ...
      layout(
           ...
           #xaxis = li"Share of household income spent on rent",
           #yaxis = "Available rental units"
           #text = ~areaName,
           #xaxis = list(title = 'Percentage of household income spent on rent'),
           #yaxis = list(title = 'Median Asking Rent'),
            )
  })
      
    #layout(
          #title='Affordability of New York City Neighborhoods',
          #subtitle='Based on median household income in New York City and neighborhood median asking rent for all rentals',
          #xaxis = list(showgrid = FALSE),
          #yaxis = list(showgrid = FALSE),
          #showlegend = FALSE,
          #xaxis_title = list('Percentage of household income spent on rent'),
          #yaxis_title = list('Median Asking Rent')
   #hovertemplate = paste(
   #  "<b>%{text}</b><br><br>",
   #  "%{xaxis.title.text}: %{x:.0%}<br>",
   #  "%{yaxis.title.text}: %{y:,.0f}<br>",
   #  "Median asking rent: %{marker.size:,}",
   #  "<extra></extra>")
   ...
   #  filter(sales_df, areaName == input$areaName & type == input$sales_type_plot & metric == input$sales_metric_plot)
   #})
